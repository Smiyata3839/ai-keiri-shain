...
function calculateDueDate(issueDate: string, paymentTerms: string | null): string {
  ...
  if (terms === "月末締め翌々月末払い") {
    return formatDate(lastDayOfMonth(d.getFullYear(), d.getMonth() + 2));
  }
...
export default function InvoiceNewPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // invoice number: check existing count for today
    ...
    <div style={{ display: "flex", minHeight: "100vh", fontFamily: "var(--font-sans)" }}>
      ...
      <div style={{ marginLeft: "240px", flex: 1, background: "var(--color-background)", minHeight: "100vh" }}>
        ...
        <div style={{
          ...
          <h2 style={{ margin: 0, fontSize: "18px", fontWeight: "700", color: "var(--color-text)" }}>請求書発行</h2>
          <p style={{ margin: 0, fontSize: "12px", color: "var(--color-text-secondary)" }}>適格請求書（インボイス）を作成します</p>
        </div>
